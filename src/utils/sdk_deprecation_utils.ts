...
const helplink: any = DefaultConstructor.liveHelpLink;

// Event name remapping
export const deprecatedEventMap: Record<string, { newKey: string | string[]; url: string | string[] }> = {
    'actionBegin': {
...
export const deprecatedMemberMap: Record<string,  { newKey: string; url: string }> = {
    'isBingMapRequired': {
...
const deprecatedMethodMap: Record<string, { newKey: string; url: string }> = {
    destroy: {
        newKey: 'dispose',
...
export function migrateDeprecatedEventKeys(embedOptions: any): any {
    ...
    Object.keys(deprecatedEventMap).forEach((oldKey) => {
        const { newKey, url } = deprecatedEventMap[oldKey];
       // const newKey = deprecatedEventMap[oldKey];
        ...
        const targets : any = Array.isArray(newKey) ? newKey : [newKey];
        const urls : any = Array.isArray(url) ? url : [url];
        targets.forEach((newKey: any, index: any) => {
            const newKeyParts: any = newKey.split('.');
...
export function checkDeprecatedEmbedOptions(options: any): void {
    Object.keys(deprecatedMemberMap).forEach((oldKey) => {
        ...
        if (found) {
            const { newKey, url } = deprecatedMemberMap[oldKey];
            if (url) {
                console.warn(
                    `[Deprecation Warning] '${oldKey}' is deprecated. Please use '${newKey}' instead. For more details, visit: %s`,
                    `${helplink}${url}`
                );
            } else {
                console.warn(
                    `[Deprecation Warning] '${oldKey}' is deprecated. Please use '${newKey}' instead.`
                );
            }
        }
    });
...
export function deprecatedMethod(  methodName: string): any {
    ...
    if (newKey) {
        if (url) {
            console.warn(
                `[Deprecation Warning] '${methodName}()' is deprecated. Please use '${newKey}()' instead. For more details, visit: %s`,
                `${helplink}${url}`
            );
        }
        else{
            console.warn(
                `[Deprecation Warning] '${methodName}()' is deprecated. Please use '${newKey}()' instead.`
            );
        }
    }
}
